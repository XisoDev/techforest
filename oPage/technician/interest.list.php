...
$total_record = count($interest_rows);
    $scale = 12;
	$start = ($page - 1) * $scale;
...
<section class="bg-white d-lg-none">
    <div class="p-3 mt-4 pt-5 mb-0 pb-2">
        ...
        <h4 class="weight_normal">관심공고 더보기</h4>
    </div>
</section>
...
<div class="container pt-lg-5">
    ...
    <div class="row">
      ...
      <?php for($i = $start; $i < $start + $scale && $i < $total_record; $i++) { ?>
        <input type="hidden" id="hidden_h_idx" value="<?=$interest_rows[$i]['h_idx']?>">
                  ...
                  <div class="row">
                      <div class="col-5 col-md-12 px-0" style="background-color:#EEE;">
                          <div class="thumbnail mx-0 px-0" style="height:100%; background-image:url('http://www.planttech.co.kr/wp-content/uploads/2018/07/%EC%82%BC%EC%84%B1%EC%97%94%EC%A7%80%EB%8B%88%EC%96%B4%EB%A7%811-820x457.png')">
                              <span class="overlay">
                                <?
                                  $interest_html2 = '<a class="btn-xxs btn btn-round border-white text-white py-1 px-2 position-absolute mr-lg-3" onclick="interest_remove('. $interest_rows[$i]['h_idx'] .')" style="right:10px; top:10px;">관심공고<i class="xi-heart red" id="yes_interest"></i></a>';
                                ?>
                                <?= $interest_html2 ?>
                              </span>
                          </div>
                      </div>
                      <? if ($interest_rows[$i]['city_name'] == "전체") { $interest_rows[$i]['city_name'] = "";} ?>
                      <? if ($interest_rows[$i]['district_name'] == "전체") { $interest_rows[$i]['district_name'] = ""; }?>
                      <? if ($interest_rows[$i]['salary_idx'] < 3) { $hire_salary_text = "만원"; } else { $hire_salary_text = "원"; } ?>
                      <div class="col-7 col-md-12 pl-0 pl-md-3">
                          <div class="p-2">
                              <h6 class="weight_normal cut1"><?=$interest_rows[$i]['c_name']?></h6>
                              <h6 class="red cut1"><?=$interest_rows[$i]['h_title']?></h6>
                              <p class="weight_lighter xxs_content mx-0 px-0">
                                  <span class="badge badge-danger weight_lighter">위치</span>
                                    <?= $interest_rows[$i]['local_name'] . " " . $interest_rows[$i]['city_name'].$interest_rows[$i]['district_name']?>
                                  <span class="badge badge-danger weight_lighter">
                                    <?if($interest_rows[$i]['salary_idx'] == "1"){
                                        echo "연봉";
                                      }else if($interest_rows[$i]['salary_idx'] == "2"){
                                        echo "월급";
                                      }else if($interest_rows[$i]['salary_idx'] == "3"){
                                        echo "일급";
                                      }else{
                                        echo "시급";
                                      }?>
                                  </span>
                                  <b><?= number_format($interest_rows[$i]['job_salary']) . $hire_salary_text?></b>
                              </p>
                              <p class="text-secondary xxs_content mx-0 px-0 pt-1">
                                  <img src="/oPage/images/imgicons/wrench_bg_red.png" height="14" /> <?=$interest_rows[$i]['job_is_career']?>
                              </p>
                          </div>

                          <div class="row m-0 p-0 pt-0 mt-0">
                              <div class="col-6 mx-0 px-0">
                                  <a href="<?=getUrl('technician','jobDetail',$interest_rows[$i]['h_idx'])?>" class="btn btn-light btn-block rounded-0">상세보기</a>
                              </div>
                              <div class="col-6 mx-0 px-0">
                                  <button class="btn btn-danger btn-block rounded-0" onclick="application_ok(<?=$interest_rows[$i]['h_idx']?>)">지원하기</button>
                              </div>
                          </div>
                      </div>
                  </div>
